...
public class TextClockSkin extends ClockSkinBase {
    ...
    @Override protected void initGraphics() {
        ...
        pane = new Pane(dateText, timeText);
    ...
    @Override protected void resize() {
        ...
        if (width > 0 && height > 0) {
            ...
            if (clock.isTextVisible()) {
                if (null == customFont) {
                    timeText.setFont(Locale.US == clock.getLocale() ? Fonts.robotoLight(0.3 * height) : Fonts.robotoLight(0.3 * height));
                } else {
                    timeText.setFont(Locale.US == clock.getLocale() ? new Font(customFont.getName(), 0.3 * height) : new Font(customFont.getName(), 0.3 * height));
                }
                timeText.setX((width - timeText.getLayoutBounds().getWidth()) * 0.5);
                if (clock.isDateVisible()) {
